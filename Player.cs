...
namespace Robotic_Mayhem
{
    static class Player
    {
        ...
        private static Vector2 handleKeyboardShots(KeyboardState keyState)
        {
            ...
            if (keyState.IsKeyDown(Keys.NumPad1) || keyState.IsKeyDown(Keys.M))
                keyShots = new Vector2(-1, 1);

            if (keyState.IsKeyDown(Keys.NumPad2) || keyState.IsKeyDown(Keys.K))
                keyShots = new Vector2(0, 1);

            if (keyState.IsKeyDown(Keys.NumPad3) || keyState.IsKeyDown(Keys.OemPeriod))
                keyShots = new Vector2(1, 1);

            if (keyState.IsKeyDown(Keys.NumPad4) || keyState.IsKeyDown(Keys.J))
                keyShots = new Vector2(-1, 0);

            if (keyState.IsKeyDown(Keys.NumPad6) || keyState.IsKeyDown(Keys.L))
                keyShots = new Vector2(1, 0);

            if (keyState.IsKeyDown(Keys.NumPad7) || keyState.IsKeyDown(Keys.U))
                keyShots = new Vector2(-1, -1);

            if (keyState.IsKeyDown(Keys.NumPad8) || keyState.IsKeyDown(Keys.I))
                keyShots = new Vector2(0, -1);

            if (keyState.IsKeyDown(Keys.NumPad9) || keyState.IsKeyDown(Keys.O))
                keyShots = new Vector2(1, -1);
